...
void set_current_key_signature(char key_signature)
{
  ...
  {
  case 'C':
    active_key = 1; // C Major
    break;
  case 'A':
    active_key = 2; // A Major
    break;
  case 'G':
    active_key = 3; // G Major
    break;
  default:
    active_key = 1; // Default to C Major
    break;
  }
}
...
{
  int current_key_signature = active_key;
...
static PT_THREAD(protothread_vga_state(struct pt *pt))
{
  ...
  while (1)
  {
    ...
    sprintf(screentext, "Current Key signature: %d", active_key);
