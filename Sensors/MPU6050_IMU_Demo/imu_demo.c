...
void on_pwm_wrap()
{
    ...
    // print accelerometer and gyro measurements
    // to the terminal

    printf("Accel[0]: %f \t", fix2float15(acceleration[0] >> 4));
    printf("Accel[1]: %f \t", fix2float15(acceleration[1] >> 4));
    printf("Accel[2]: %f \n", fix2float15(acceleration[2] >> 4));

    // sprintf(pt_serial_out_buffer, "gyro: %d, %d, %d\n", gyro[0], gyro[1], gyro[2]);
    // Signal VGA to draw
