...
// angle parameters
fix15 desired_angle = int2fix15(180); // Initial desired angle is 90 degrees
fix15 error_angle;
fix15 filtered_ay;
fix15 filtered_az;
...
fix15 proportional_control;
fix15 integral_control;
fix15 derivative_control;
...
void on_pwm_wrap()
{
    ...
    filtered_ay += (acceleration[1] - filtered_ay) >> 1;
    filtered_az += (acceleration[2] - filtered_az) >> 1;
    ...
    proportional_control = multfix15(int2fix15(K_p), error_angle);

    // control is the sum of the proportional, integral, and derivative control inputs
    control = fix2int15(proportional_control);
...
static PT_THREAD(protothread_serial_pid(struct pt *pt))
{
    PT_BEGIN(pt);
    static int test_in;
    static int user_angle, user_kp, user_ki, user_kd;
    static int choice;
    while (1)
    {
        ...
        case 2:
            ...
            serial_read;
            // convert input string to number
            sscanf(pt_serial_in_buffer, "%d", &user_kp);
            K_p = user_kp;
            break;
        case 3:
            // Get K_i from user
            ...
            serial_read;
            // convert input string to number
            sscanf(pt_serial_in_buffer, "%d", &user_ki);
            K_i = user_ki;
            break;
        case 4:
            // Get K_d from user
            ...
            serial_read;
            // convert input string to number
            sscanf(pt_serial_in_buffer, "%d", &user_kd);
            K_d = user_kd;
            break;
        default:
...
static PT_THREAD(protothread_vga(struct pt *pt))
{
    ...
    while (true)
    {
        ...
        if (throttle >= threshold)
        {
            ...
            sprintf(screentext, "Error: %0.4f", fix2float15(error_angle));
            writeString(screentext);
            ...
            sprintf(screentext, "Control: %d", control);
            writeString(screentext);
            ...
            sprintf(screentext, "I: %d", fix2int15(integral_control));
