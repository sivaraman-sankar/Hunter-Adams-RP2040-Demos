...
volatile s15x16  low_pass_coeff = float_to_s15x16(0.9) ; // close to classic Karplus Strong
...
// time since pluck
int sample_number ;
s15x16 drive_amp = float_to_s15x16(0.0);
// rise/fall time of impulse in samples
s15x16 drive_rise=int_to_s15x16(10000), drive_fall=int_to_s15x16(4000), drive_rise_inc, drive_fall_inc ;
...
void print_all(){
  ...
  sprintf(print_str, "bow attack=%5.3f   ", s15x16_to_float(drive_rise)/Fs);
  writeStringBig(print_str) ;

  setCursor(print_col[1], print_row[2]);
  sprintf(print_str, "bow decay=%5.3f   ", s15x16_to_float(drive_fall)/Fs);
  writeStringBig(print_str) ;

  setCursor(print_col[2], print_row[2]);
  sprintf(print_str, "bow amp=%5.0f   ", s15x16_to_float(drive_amp));
  writeStringBig(print_str) ;
...
void compute_sample(void)
{
    ...
    if ((sample_number)<s15x16_to_int(drive_rise + drive_fall)){
        ...
        string_input = muls15x16((current_drive_amp)>>10 , init_string[1][sample_number % (string_length[current_note]-2)] );
      ...
      string[ptrin] = tuning_out + (string_input>>3) ; ///(ptrin==20)?(string_input) : 0 ;
 ...
 gpio_set_dir(2, GPIO_OUT) ;
  ...
  for (int i=0; i<max_string_size; i++){
   ...
   // Gaussian pluck
    init_string[2][i] = float_to_s15x16(2000 * exp(-(float)(i - pluck_pos)*(i - pluck_pos)/(pluck_width * pluck_width)));  
    // sawtooth
    init_string[3][i] = (i<11)?int_to_s15x16(i * 200) : int_to_s15x16(2000-(i-10)*20);
	}
