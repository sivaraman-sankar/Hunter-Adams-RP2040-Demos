...
int current_pluck = 2 ;
// gaussian
int pluck_pos = 20, pluck_width = 1, output_pos = 15 ;
...
// low pass coeff units are amount of right-shift in 
// (((string[ptrin] - string[ptrout])>>lp_coeff) + string[ptrout])
volatile char  low_pass_coeff = 1 ; // close to classic Karplus Strong

// damping coefficient reduction in amp per synthesis sample
// implies a e-fold decay time constant of 100 samples 
// at 20kHz, 5 mSec
volatile s15x16  damping_coeff = float_to_s15x16(0.99)  ;

// driving the string (as if bowing)
// time since pluck
int sample_number ;
s15x16 drive_amp = float_to_s15x16(500.0);
// rise/fall time of impulse in samples
s15x16 drive_rise=int_to_s15x16(15000), drive_fall=int_to_s15x16(100), drive_rise_inc, drive_fall_inc ;
s15x16 current_drive_amp, string_input ;
//volatile int impulse_duration = 100 ;


///////////////////////////////////////////////
//controls needed:
// pluck  amp, type, width (if gaussian)
//  0 to 1, 0 to 3, 1 to 20
// damping coeff 0.9 t0 1.0
// lowpass coeff 1 to 6
// note rate mSec
// drive attack time, decay time, type, amp
// sec, sec, 0 to 3, 0 to 1
// 
///////////////////////////////////////////////
...
void print_all(){
  ...
  sprintf(print_str, "Octave=%d   ", octave_num);
  writeStringBig(print_str) ;
  setCursor(print_col[1], print_row[0]);
  sprintf(print_str, "MaxAmp=%5.0f   ", fix_to_float(max_amp));
  writeStringBig(print_str) ;
  setCursor(print_col[0], print_row[1]);
  sprintf(print_str, "ModFreqRatio=%5.3f   ", (Fmod));
  writeStringBig(print_str) ;
  setCursor(print_col[1], print_row[1]);
  sprintf(print_str, "ModDepth=%5.3f   ", fix_to_float(max_mod_depth)/100000);
  writeStringBig(print_str) ;
  //
  setCursor(print_col[0], print_row[2]);
  sprintf(print_str, "Attack=%5.3f   ", fix_to_float(attack_time)/Fs);
  writeStringBig(print_str) ;
  setCursor(print_col[1], print_row[2]);
  sprintf(print_str, "Sustain=%5.3f   ", fix_to_float(sustain_time)/Fs);
  writeStringBig(print_str) ;
  setCursor(print_col[2], print_row[2]);
  sprintf(print_str, "Decay=%5.3f   ", fix_to_float(decay_time)/Fs);
  writeStringBig(print_str) ;
//
setCursor(print_col[0], print_row[3]);
  sprintf(print_str, "ModAttack=%5.3f   ", fix_to_float(mod_attack_time)/Fs);
  writeStringBig(print_str) ;
  setCursor(print_col[1], print_row[3]);
  sprintf(print_str, "ModSustain=%5.3f   ", fix_to_float(mod_sustain_time)/Fs);
  writeStringBig(print_str) ;
  setCursor(print_col[2], print_row[3]);
  sprintf(print_str, "ModDecay=%5.3f   ", fix_to_float(mod_decay_time)/Fs);
  writeStringBig(print_str) ;
}
...
static PT_THREAD (protothread_FM(struct pt *pt))
{
  PT_BEGIN(pt);
    ...
    // == Fout and Fmod are in Hz
      ...
      octave_num = 3;
      Fmod =  3 ;
      static float current_note ;
      static float farg ;
      // fm modulation strength
      max_mod_depth = float_to_fix(0.25 * 100000) ;

      // convert main input times to sample number
      attack_time = float_to_fix(0.001 * Fs) ;
      decay_time = float_to_fix(1.0 * Fs) ;
      sustain_time = float_to_fix(0.1 * Fs);
      ...
      // modulation times
      mod_attack_time = float_to_fix(.001 * Fs) ;
      mod_decay_time = float_to_fix(0.2 * Fs) ;
      mod_sustain_time = float_to_fix(0.1 * Fs) ;
      // and now get increments
    ...
    while(1) {
      ...
      // print prompt
        sprintf(pt_serial_out_buffer, "FM cmd> ");
      ...
      // parse by command
        if(strcmp(cmd,"help")==0){
            ...
            printf("***\n\rplay -- play scale\n\r");          
            printf("stop -- playng\n\r");
            printf("octave number -- 1 to 8 -- C4 is middle C\n\r") ;
            printf("maxamp level -- 0 to 2000 -- DAC units\n\r") ;  
            printf("wave type -- sine, saw, square \n\r") ;  
            printf("attack time -- rise time in seconds\n\r");
            printf("sustain time -- hold time in seconds\n\r");
            printf("decay time -- fall time in seconds\n\r"); 
            printf("mfreq ratio -- modulation frequency as fraction of note frequency\n\r"); 
            printf("mdepth depth -- 0 to 5 or so -- intensity of modulation\n\r");
            printf("mwave type -- sine, saw, square \n\r") ; 
            printf("mattack time -- modulation rise time in seconds\n\r");
            printf("msustain time -- modulation hold time in seconds\n\r");
            printf("mdecay time -- -- modulation fall time in seconds\n\r") ;
            printf("***\n\r");
        }
        //
        ...
        else if(strcmp(cmd,"octave")==0){
            sscanf(arg1,"%d", &octave_num) ;
            // get the DDS increments
            for(int i=0; i<8; i++){
                current_note = notes[i] * pow(2, octave_num-3) ;
                main_inc[i] = current_note * pow(2,32 )/ Fs ;
        ...
        else if(strcmp(cmd,"maxamp")==0){
            sscanf(arg1,"%f", &farg) ;
            max_amp = float_to_fix(farg) ;
            attack_inc = div(max_amp, attack_time) ;
            // linear and parabolic fit
            decay_inc = div(max_amp, decay_time) ; 
            recip_decay_time = div(onefix, decay_time);
            ...
            sscanf(arg1, "%f", &farg);
            attack_time = float_to_fix(farg * Fs) ;
            attack_inc = div(max_amp, attack_time) ;
        }

        else if(strcmp(cmd,"sustain")==0){
            sscanf(arg1, "%f", &farg);
            sustain_time = float_to_fix(farg * Fs) ;
        }

        else if(strcmp(cmd,"decay")==0){
            sscanf(arg1, "%f", &farg);
            decay_time = float_to_fix(farg * Fs) ;
            decay_inc = div(max_amp, decay_time) ;
        }

        else if(strcmp(cmd,"mfreq")==0){
            sscanf(arg1, "%f", &farg);
            Fmod = farg ;
            for(int i=0; i<8; i++){
                current_note = notes[i] * pow(2, octave_num-3) ;
        ...
        else if(strcmp(cmd,"mdepth")==0){
            sscanf(arg1, "%f", &farg);
            max_mod_depth = float_to_fix(farg * 100000) ;
            // precomputing increments means that only add/subtract is needed
            mod_attack_inc = div(max_mod_depth, mod_attack_time) ;
            mod_decay_inc = div(max_mod_depth, mod_decay_time) ;
        }

        else if(strcmp(cmd,"mattack")==0){
            sscanf(arg1, "%f", &farg);
            mod_attack_time = float_to_fix(farg * Fs) ;
            attack_inc = div(max_mod_depth, mod_attack_time) ;
        }

        else if(strcmp(cmd,"msustain")==0){
            sscanf(arg1, "%f", &farg);
            mod_sustain_time = float_to_fix(farg * Fs) ;
        }

        else if(strcmp(cmd,"mdecay")==0){
            sscanf(arg1, "%f", &farg);
            mod_decay_time = float_to_fix(farg * Fs) ;
            mod_decay_inc = div(max_mod_depth, mod_decay_time) ;
        }
...
static PT_THREAD (protothread_play(struct pt *pt))
{
    PT_BEGIN(pt);
     ...
     drive_rise_inc = divs15x16(drive_amp<<4, drive_rise) ;
     drive_fall_inc = divs15x16(drive_amp<<4, drive_fall) ;
...
void compute_sample(void)
{
    ...
    // ==== play the KS ===
      ...
      // input calculations
    if ((sample_number)<s15x16_to_int(drive_rise + drive_fall)){
        current_drive_amp = ((sample_number) <= s15x16_to_int(drive_rise))? 
        ...
        string_input = muls15x16((current_drive_amp)>>10 , init_string[0][sample_number % (string_length[current_note])] );
        //string_input = init_string[0][sample_number % max_string_size] ;
        //input = drive_table[impulse_drive][local_number] ;
    }
    else {
        string_input = 0 ;
    }
      ...
      string[ptrin] = tuning_out + (string_input>>9) ; ///(ptrin==20)?(string_input) : 0 ;
