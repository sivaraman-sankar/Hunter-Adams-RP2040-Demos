...
void beep()
{
    // Start the control channel, which will trigger one sine wave output
    dma_channel_start(1);

    // // Wait until data channel completes (blocking call)
    // while (dma_channel_is_busy(data_chan)) {
    //     tight_loop_contents();  // Keep CPU busy while DMA runs
    // }
}
